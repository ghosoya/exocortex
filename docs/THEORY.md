...
```text
               ┌────────────────────────────────────────────────────────┐
               │              SYMBIOTIC COGNITIVE RESONANCE             │
               │                                                        │
┌──────────────┴──────────────┐                          ┌──────────────┴──────────────┐
│          OPERATOR           │   Teleological Vectors   │          EXOCORTEX          │
│   (Teleology, Ground Truth, ├─────────────────────────►│  (Inference Engine, Guards, │
│       Sovereign Veto)       │◄─────────────────────────┤    Topological Phase Space) │
...
* **Exocortex Invariance (Popperian):** An architecture is defined strictly by its constraints. A `BoundaryConstraint` explicitly forbids invalid states (e.g., side effects across bounded contexts, global state mutations, orphan tool calls).
...
```text
   Boundary Constraint (Inviolable Prohibition Boundary)
   ═══════════════════════════════════════════════════════════════════════
                      \                               /
                       \     Trajectory Operator     /
                        \           (TO)            /
                         \                         /
                          \    ┌────────────────┐ /
                           \   │ Potential Well │/ (Attractor Basin)
                            ▼  │    (PW_001)    │
                               └────────────────┘
...
```text
           ...
           ▼                                                       ▼
┌──────────────────────────────────────┐        ┌──────────────────────────────────────┐
│       High-Density Attractor         │        │           Defensive Sprawl           │
│ - Formal Invariants (BC, PW, TO)     │        │ - Conversational boilerplate         │
│ - Zero Decorative Prose              │        │ - Redundant try-except wrappers      │
│ - High Attractor Gravity             │        │ - Unconstrained semantic drift       │
│ (< 350 Tokens)                       │        │ (> 2500 Tokens)                      │
└──────────────────────────────────────┘        └──────────────────────────────────────┘
...
│                                                                        │
│   ┌──────────────┐     Mutation Tool     ┌──────────────┐              │
│   │  LLM Turn /  ├──────────────────────►│ Graph Store  │              │
│   │  Reasoning   │                       │  (NetworkX)  │              │
│   └──────▲───────┘                       └──────┬───────┘              │
│          │                                      │                      │
│          │ Rehydration / Dynamic Context        │ Auto-Projection      │
│          │                                      ▼                      │
│   ┌──────┴───────┐     Visual Audit      ┌──────────────┐              │
│   │   Operator   │◄──────────────────────┤   Obsidian   │              │
│   │ Intervention │                       │   .canvas    │              │
